...
class Events(commands.Cog):
    ...
    async def on_command_error(self, ctx, error):
        ...
        if isinstance(error, commands.errors.MissingRequiredArgument):
            ...
            error_embed = discord.Embed(title="There was an error!", description=f"Missing parameter: `{param}`", colour=discord.Color.dark_blue())
            await ctx.reply(embed=error_embed)
        if isinstance(error, commands.errors.CommandInvokeError):
            error_embed = discord.Embed(title="Unknown error!", description=f"```{error}```", colour=discord.Color.dark_blue())
            await ctx.reply(embed=error_embed)
            print(error)
