...
class Song:
    ...
    def create_embed(self):
        em = (discord.Embed(description='```css\n{0.source.title}\n```'.format(self), color=discord.Color.blurple())
                 ...
                 .set_footer(text=f"Requested by {self.requester.name} (In streaming)", icon_url=f"{self.requester.avatar.url}"))
...
# Search whatever u want on youtube!
    ...
    async def syt(self, ctx, *, search):
        ...
        em.set_footer(text=f"Search requested by {ctx.author.name}", icon_url=f"{ctx.author.avatar.url}")
        await ctx.send(embed=em)
