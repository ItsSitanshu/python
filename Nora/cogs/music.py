...
class Player(wavelink.Player):
    ...
    def build_embed(self) -> typing.Optional[discord.Embed]:
        ...
        embed = discord.Embed(title=f'Music Controller | {channel.name}', colour=discord.Color.dark_blue())
...
class PaginatorSource(menus.ListPageSource):
    ...
    async def format_page(self, menu: menus.Menu, page):
        embed = discord.Embed(title='Coming Up...', colour=discord.Color.dark_blue())
...
class Music(commands.Cog, wavelink.WavelinkMixin):
    ...
    async def play(self, ctx: commands.Context, *, query: str):
        ...
        if isinstance(tracks, wavelink.TrackPlaylist):
            ...
            await ctx.send(f'Added the playlist {tracks.data["playlistInfo"]["name"]} with {len(tracks.tracks)} songs to the queue.', delete_after=15)
        else:
    ...
    async def stop(self, ctx: commands.Context):
        ...
        if self.is_privileged(ctx):
            ...
            return await player.teardown(ctx)
