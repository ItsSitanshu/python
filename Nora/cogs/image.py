...
class Images(commands.Cog):
    ...
    async def _avatar(self, ctx, *, member: discord.Member = None):
        ...
        pngurl = str(member.avatar.replace(format="png", static_format="png", size=1024))
        jpgurl = str(member.avatar.replace(format="jpg", static_format="jpg", size=1024))
        jpegurl = str(member.avatar.replace(format="jpeg", static_format="jpeg", size=1024))
        webpurl = str(member.avatar.replace(format="webp", static_format="webp", size=1024))
        e = discord.Embed(title=f"{member}'s avatar", description=f"[`PNG`]({pngurl}) | [`JPG`]({jpgurl}) | [`JPEG`]({jpegurl}) | [`WEBP`]({webpurl})", color=member.color)
        e.set_image(url=f"{member.avatar.url}")
    ...
    async def _america(self, ctx, *, member: discord.Member = None):
        ...
        if bl_users:
            ...
            await ctx.send(embed=e)
            return
        e = discord.Embed(title="This may take some time", color=discord.Color.dark_blue())
    ...
    async def comment(self, ctx, member: discord.Member, *,msg):
        ...
        if not member:
            member = ctx.author
        pfp = member.avatar.replace(format='png')
